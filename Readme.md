...
2. Compile `maze.c` using the command: `clang -O3 -o maze maze.c -lm`.
3. Create a copy of the `maze.c` file and name it `maze_optimized.c`.
4. Open `maze_optimized.c` and modify the code to optimize the program, aiming to improve the `seconds time elapsed` measurement for the command `perf stat ./maze_optimized 16384 16384 maze_optimized.pbm` compared to `perf stat ./maze 16384 16384 maze.pbm`. Compile the optimized code with: `clang -O3 -o maze_optimized maze_optimized.c -lm`.
5. Verify the output files are identical. For small test runs, you can use `vimdiff maze.pbm maze_optimized.pbm`. For the full 16384 by 16384 output, use `diff maze.pbm maze_optimized.pbm` or compare their hashes with `sha256sum maze.pbm maze_optimized.pbm`. The files MUST be identical.
...
* Do NOT use any additional compiler flags aside from `-O3` and `-lm`.
...
* Do NOT use inline assembly, compiler intrinsics, or third-party libraries to accelerate your code. You may modify the stack's underlying implementation, optimize the maze generation, delete unnecessary code, or reorganize it. Any function from the C standard library or POSIX available on Ubuntu 24.04 may be used. You may also reimplement standard library function logic directly in your code.
...
4. You may also achieve a significant performance boost by optimizing input, output, or allocation logic. Use your knowledge from previous courses (such as Principles of Computing Systems, Operating Systems, or other) to assist you here. Keep in mind that some standard library calls can be inlined or replaced with lower-level POSIX/OS calls available on Ubuntu 24.04 for better performance.
...
Commit and push your changes to the private GitHub repository provided by your instructor. Do NOT attempt to upload the generated `.pbm` files. Submit the URL pointing to the latest commit to Moodle before the deadline. The URL MUST include the commit hash. Remove all `.pbm` files from your home directory after submission.
